...
(defun ed25519-edwards-double (p)
  ...
  (let* ((x (aref p 0))
         ...
         (f (mod (+ c d) +ed25519-q+))
         (g (mod (* z z) +ed25519-q+))
         (h (mod (- (+ g g) e) +ed25519-q+))
         (u (mod (* (- b f) h) +ed25519-q+))
         (v (mod (* e f) +ed25519-q+))
         (w (mod (* e h) +ed25519-q+)))
    (declare (type integer x y z a b c d e f g h u v w))
    (vector u v w)))
...
(defun ed25519-edwards-add (p q)
  ...
  (let* ((x1 (aref p 0))
         ...
         (w (mod (* f g) +ed25519-q+)))
    (declare (type integer x1 y1 z1 x2 y2 z2 a b c d e f g h j u v w))
    (vector u v w)))
